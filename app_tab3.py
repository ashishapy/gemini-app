...
def render_image_playground_tab(multimodal_model_pro: GenerativeModel):
    ...
    with diagrams:
        ...
        st.image(er_diag_url, width=350, caption="ER Diagram")
        ...
        with tab1:
            if er_diag_img_description and prompt:
                with st.spinner("Generating documentation using Gemini..."):
    ...
    with equations:
        ...
        st.markdown(f"""
                    Ask question about math equation as follows:
                    - Extract the formula
                    - What is symbol right before Pi? What does it mean?
                    - Is this s famous formula? Does it have a name? """)
        
        prompt = """ Follow the instructions. Surround the math equation with $. Use a table with a row for each instruction and its result.
                    INSTRUCTIONS:
                    - Extract the formula
                    - What is symbol right before Pi? What does it mean?
                    - Is this s famous formula? Does it have a name?
