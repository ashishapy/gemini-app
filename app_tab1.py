...
def generate_prompt():
    ...
    First start by giving the book introduction, chapter introduction then each chapter. It should also have proper ending.
    The book should have a prolouge and epilogue.
    """

    return temprature, prompt

# Function to render the story tab, and call the model, and display the model prompt and response
def rendor_story_tab(text_model_pro: GenerativeModel):
    st.write("Using Gemini 1.0 Pro - Text only model")
    st.subheader("Generate a story")

    temprature, prompt = generate_prompt()
