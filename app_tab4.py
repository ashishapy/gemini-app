...
# Render the Video Playgroud tab with mutile child tabs
def render_video_playground_tab(multimodal_model_pro: GenerativeModel):
    ...
    video_desc, video_tags, videos_highlights, video_geoloc = st.tabs(["Video description", "Video tags", "Video highlights", "Video geolocation"])
    ...
    with video_tags:
        ...
        prompt = """Answer following questions using the video only:
                ...
                Write the answer in table format with question and answer in columns."""
        
        tab1, tab2 = st.tabs(["Response", "Prompt"])
        video_tag_desc = st.button("Generate video tags", key="video_tag_desc")

        with tab1:
            if video_tag_desc and prompt:
    ...
    with videos_highlights:
        ...
        prompt = """Answer the following questions using video only:
        What is the profession of the girl in this video?
        Which features of the phone highlighted here?
        Summarize the video in 1 paragraph.
